...
const VideoPlayer = forwardRef<PlayerHandle, Props>(({
  ...
  useEffect(() => {
    ...
    // diagnostics — helps see which playback path a browser (e.g. Zen) takes
    console.info('[SyncWatch] HLS detect:', {
      hlsSupported: Hls.isSupported(),
      hasMediaSource: typeof (window as any).MediaSource !== 'undefined',
      hasManagedMSE: typeof (window as any).ManagedMediaSource !== 'undefined',
      canPlayNativeHls: video.canPlayType('application/vnd.apple.mpegurl'),
      src,
    })

    // Prefer hls.js wherever MSE exists (Chrome, Firefox, Zen, Edge…). Fall back to
    // native HLS only when hls.js can't run (iOS Safari, no MSE). Don't check
    // canPlayType first — Firefox/Zen return 'maybe' for the HLS MIME yet can't play
    // it natively, which caused "no video with supported format".
    if (!Hls.isSupported()) {
